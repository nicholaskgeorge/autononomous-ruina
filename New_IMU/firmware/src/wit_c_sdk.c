...
int32_t WitWriteReg(uint32_t uiReg, uint16_t usData)
{
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_I2C:
            ...
            ucBuff[1] = usData >> 8;
            p_WitI2cWriteFunc(s_ucAddr, uiReg, ucBuff, 2);
        break;
		default:return WIT_HAL_ERROR;
    }
    return WIT_HAL_OK;
}
int32_t WitReadReg(uint32_t uiReg, uint32_t uiReadNum)
{
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_I2C:
            ...
            if(p_WitI2cReadFunc(s_ucAddr, uiReg, s_ucWitDataBuff, usTemp) == 1)
            {
                if(p_WitRegUpdateCbFunc == NULL)return WIT_HAL_EMPTY;
                
                for(i = 0; i < uiReadNum; i++)
                {
                    sReg[i+uiReg] = ((uint16_t)s_ucWitDataBuff[i<<1] << 8) | s_ucWitDataBuff[(i<<1)+1];
                }
...
/*change Bandwidth demo*/
int32_t WitBandwidthDemo(int32_t uiBaudWidth)
{	
	if(!CheckRange(uiBaudWidth,BANDWIDTH_256HZ,BANDWIDTH_5HZ))
	{
		printf("\r\nPlease Check Input BaudWidth\r\n");
		return WIT_HAL_INVAL;
	}
	if(WitWriteReg(KEY, KEY_UNLOCK) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
	delay_ms(200);
...
/*change RSW demo*/
int32_t WitRswDemo(int32_t uiRsw)
{	
	if(!CheckRange(uiRsw,RSW_TIME,RSW_MASK))
	{
		printf("\r\nPlease Check Input RSW\r\n");
		return WIT_HAL_INVAL;
	}
	if(WitWriteReg(KEY, KEY_UNLOCK) != WIT_HAL_OK)	return  WIT_HAL_ERROR;
	delay_ms(200);
